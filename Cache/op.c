...
void file_output()
{
	...
	uint64_t miss_rate = (CACHE[L1].CACHE_STAT.num_read_misses + CACHE[L1].CACHE_STAT.num_write_misses) / (CACHE[L1].CACHE_STAT.num_reads + CACHE[L1].CACHE_STAT.num_writes);
	fprintf(fp, "e. L1 miss rate:              %llu\n", miss_rate);
	fprintf(fp, "f. number of L1 writebacks:   %llu\n", CACHE[L1].CACHE_STAT.num_write_backs);
	fprintf(fp, "g. number of L2 reads:        %llu\n", CACHE[L2].CACHE_STAT.num_reads);
	fprintf(fp, "h. number of L2 read misses:  %llu\n", CACHE[L2].CACHE_STAT.num_read_misses);
	fprintf(fp, "i. number of L2 writes:       %llu\n", CACHE[L2].CACHE_STAT.num_writes);
	fprintf(fp, "j. number of L2 write misses: %llu\n", CACHE[L2].CACHE_STAT.num_write_misses);
	miss_rate = (CACHE[L2].CACHE_STAT.num_read_misses + CACHE[L2].CACHE_STAT.num_write_misses) / (CACHE[L2].CACHE_STAT.num_reads + CACHE[L2].CACHE_STAT.num_writes);
	fprintf(fp, "k. L2 miss rate:              %llu\n", miss_rate);
	fprintf(fp, "l. number of L2 writebacks:   %llu\n", CACHE[L2].CACHE_STAT.num_write_backs);
	fprintf(fp, "m. total memory traffic:      %llu\n", CACHE[L2].CACHE_STAT.num_blocks_transferred);
